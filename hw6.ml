...
let unused_vars_tests = [
  ...
  *)
  (Let ("x", I 1, I 5), ["x"]);
  (Rec ("f", Int, I 5), ["f"]);
  (Rec ("f", Int, Fn(["x", Int], Apply(Var "f", [Var "x"]))), []);
  (Fn ([("x", Int); ("y", Int)], I 5), ["x"; "y"]);
  (Apply(Var "f", [Var "x"; Var "y"]), []);
  (Let ("f", Fn ([("x", Int); ("y", Int)], I 5), Apply(Var "f", [I 0; I 0])), ["x"; "y"]); 
]
...
let rec unused_vars =
  ...
  | Fn (xs, e) -> 
      let unused = unused_vars e in
      let rec checkAll list = match list with  
        | [] -> unused
        | (y, _)::ys ->
  ...
  | Apply (e, es) -> 
      let unused = unused_vars e in
      let rec checkAll list acc = match list with
        | [] -> 
            let rec checkAll' list' = match list' with
              | [] -> 
                  acc 
...
let subst_tests : (((exp * name) * exp) * exp) list = [
  ...
  *)
  ...
  (((I 1, "x"),
    Rec ("t", Int, Apply(Var "t", [Var "x"; Var "y"]))),
   Rec ("t", Int, Apply(Var "t", [I 1; Var "y"])));
  ...
  (((Var "f", "t"),
    Rec ("t", Int, Apply(Var "t", [Var "x"; Var "y"]))),
   Rec ("f", Int, Apply(Var "f", [Var "x"; Var "y"]))); 
  
  (((Primop(Plus, [Var "y"; I 1]), "x"),
    Rec ("y", Int, Var "x")),
   Rec("a", Int, Primop(Plus, [Var "y"; I 1])));
  
  (((I 1, "x"),
    Fn ([("y", Int)], Primop (Plus, [Var "x"; I 1]))),
   Fn ([("y", Int)], Primop (Plus, [I 1; I 1])));
  ...
  (((Primop(Plus, [Var "y"; I 1]), "x"),
    Fn ([("y", Int)], Var "x")),
   Fn ([("a", Int)], Primop(Plus, [Var "y"; I 1])));
  
  (((I 1, "x"),
    Apply(Var "t", [Var "x"; Var "y"])),
   Apply(Var "t", [I 1; Var "y"]));
  
  (((I 1, "x"),
    Apply(Var "t", [Var "x"; Var "y"; Apply(Var "s", [Var "a"; Var "x"])])),
   Apply(Var "t", [I 1; Var "y"; Apply(Var "s", [Var "a"; I 1])]))
]
...
let rec subst ((e', x) as s) exp =
  ...
  | Fn (xs, e) ->
      if List.mem x (List.map (fun (a, _) -> a) xs) then
        Fn (xs, e)
      else
        let rec checkAll list modList toRename = match list with
          | [] ->
              let (toRename, e) = 
                rename_all toRename e
              in
              Fn (
                (let rec update modList' toRename' = match modList' with 
                    | [] -> []
                    | (m1, m2, m3)::ms -> 
                        if m3 = true then
                          match toRename' with
                          | [] -> []
                          | (r1::rs) -> (r1, m2) :: (update ms rs)
                        else
                          (m1, m2) :: update ms toRename' 
                 in
                 List.rev (update modList toRename)) 
              , subst s e) 
          | (y1, y2)::ys ->
              if List.mem y1 (free_variables e') then
                checkAll ys ((y1, y2, true)::modList) (y1::toRename)
              else
                checkAll ys ((y1, y2, false)::modList) toRename
        in
        checkAll xs [] []
  ...
  | Apply (e, es) -> 
      let rec checkAll list = match list with
        | [] -> []
        | x::xs -> subst s x :: checkAll xs
      in 
      Apply (subst s e, checkAll es)
...
let eval_tests = [
  ...
  *)
  ...
  (Rec ("f", Int, I 6), I 6);
  (Apply (Rec("f", Int, Fn([("x", Int)], If(Primop(Equals, [Var "x"; I 0]),
                                            I 0, Primop(Plus, [I 2; Apply(Var "f", [Primop (Plus, [Var "x"; I (-1)])])])                                                  
                                           ))), [I 2]), I 4);
  (Apply (Fn([("x", Int); ("y", Int)], Primop(Plus, [Var "x"; Var "y"])), [I 2; I 2]), I 4); 
...
*)
  ...
  | Apply (e, es) ->
      match e with 
      | Fn (e1, e2) ->
          if List.length es = List.length e1 then
            eval (subst_list (List.combine es (List.map (fun (a, _) -> a) e1)) e2)
          else
            raise (Stuck (Arity_mismatch)) 
      | Let (x, e1, e2) ->
          let e1 = eval e1 in 
          eval (Apply ((subst (e1, x) e2), es))
      | If (e1, e2, e3) -> 
          (match eval e1 with
           | B true -> eval (Apply(e2, es))
           | B false -> eval (Apply(e3, es))
           | _ -> raise (Stuck If_non_true_false))
      | Rec (f, t, e') ->
          eval (Apply((subst (Rec (f, t, e'), f) e'), es)) 
...
let infer_tests = [
  ...
  *)
  (([("x", Int)], Var "x"), Int);
  ((([]), Fn([], I 0)), Arrow([], Int));
  ((([("x", Int)]), Fn([("x", Int)], I 0)), Arrow([Int], Int));
  ((([("x", Int); ("y", Int)]), Fn([("x", Int); ("y", Int)], I 0)), Arrow([Int; Int], Int));
  ((([("f", Int)]), Rec("f", Int, I 0)), Int);
  ((([]), Apply (Fn ([], I 1), [])), Int); 
  ((([("x", Int)]), Apply (Fn([("x", Int)], I 0), [Var "x"])), Int); 
  ((([("x", Int); ("y", Int)]), Apply (Fn([("x", Int); ("y", Int)], I 0), [Var "x"; Var "y"])), Int); 
...
let rec infer ctx e =
  ...
  | Fn (xs, e) -> Arrow ((List.map (fun (_, a) -> a) xs), infer (extend_list ctx xs) e)
  ...
  | Apply (e, es) ->
      ...
      | Arrow(tl, t) ->
          let rec checkAll list = match list with
            | [] ->  []
            | x::xs -> infer ctx x :: (checkAll xs)
          in
          let rec checkAll' list' list'' = match (list', list'') with
            | ([], []) ->
                t
            | (l'::ls', l''::ls'') ->
                if l' = l'' then
